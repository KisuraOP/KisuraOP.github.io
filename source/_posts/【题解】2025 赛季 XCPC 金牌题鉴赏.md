...
### [C. Stonebag](https://qoj.ac/contest/3169/problem/15434)
...
等 Alice 合并完 $a_{i-1}$ 和 $a_i$ 后，站在 Bob 的角度，要么切断 $(a_{i-2}, a_{i-1})$，要么切断 $(a_{i}, a_{i+1})$。如果切在其它地方，只会更劣。

切断 $(a_{i-2}, a_{i-1})$，相当于保留了 $i-1$ 开头的后缀；切断 $(a_{i}, a_{i + 1})$，相当于保留了 $i$ 结尾的前缀。
...
- 先手合并最后两项得到 $a_{i-2}+a_{i-1}+a_i+2$，后手切断 $(a_{i-3},a_{i-2})$，结束。
- 先手合并 $a_{i-3}$ 和 $a_{i-2}$。如果后手切断 $(a_{i-2},a_{i-1})$，变成子问题，答案是 $pre[i-2]$；如果后手切断 $(a_{i-4},a_{i-3})$，先手就把 $a_{i-3}\sim a_i$ 整段合并了，结束。
