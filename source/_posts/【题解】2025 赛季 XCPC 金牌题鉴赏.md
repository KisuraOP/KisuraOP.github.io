...
*Conclution.* 在模 $2^k$ 乘法群中，任何元素的阶都是 $2$ 的幂。

*Prove.* 由群论中拉格朗日定理的推论，有限群 $G$ 中每个元素的阶都整除群 $G$ 的阶。因此对于任意 $a\in (\mathbb{Z}/2^k)$，其阶 $d$ 一定有 $d\mid \varphi(2^k)$，也就是 $d\mid 2^{k-1}$。此时 $d$ 一定是 $2$ 的幂。

*Conclution.* 令 $x$ 为模 $2^k$ 下 $a$ 的阶，$y$ 为模 $2^{k+1}$ 下 $a$ 的阶，那么 $y$ 只可能是 $x$ 或 $2x$。

*Prove.* 由 $a^y\equiv 1\pmod{2^{k+1}}$，则一定有 $a^y\equiv 1\pmod {2^k}$，又 $x$ 是最小的满足 $a^x\equiv 1\pmod{2^k}$ 的正整数，于是 $x\mid y$。令 $a^x=1+2^kt$，那么
$$
