...
题解做法比较巧妙：先找出 $k$ 个分属不同集合的点，具体的，先钦定 $p_1=1$，然后找到离 $p_1$ 最远的点 $p_2$，这两个点肯定在不同的集合，再找到离 $p_1$ 和 $p_2$ 最远的 $p_3$，以此类推找全 $k$ 个点。剩下的 $n-k$ 个点只用看离哪个点更近就属于哪个集合。暴力是 $O(nk^2)$，可以优化到 $O(nk)$。
