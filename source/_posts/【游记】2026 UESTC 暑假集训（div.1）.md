...
E 是按位去询问，印象中第二次见，因为普通二分如果问到 $0$ 就没办法判断 $1$ 在哪一边。
...
D 貌似是先用多项式表示，答案是 $Ans=[x^k]\left(\sum\limits_{i=1}^{c}f_i x^i \cdot \prod_{t=c}^{n-1}(x+t)\right)$，需要部分分式分解 + 多点求值，比较困难，有这时间貌似可以先去补一下昨天的 CF。
