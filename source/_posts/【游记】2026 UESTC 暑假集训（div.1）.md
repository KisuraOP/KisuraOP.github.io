...
C 的关键是注意到只用为每一个 $P_i\in [0,n)$ 选定一个到 $Q_i\in [0,n)$ 的双射，而不用关注每一组 $(P_i,Q_i)$ 的顺序。将 $(P_i,Q_i)$ 按是否需要被进位，和是否有进位，能分成四个等价类。按如下顺序排布即可（第一行是被进位，第二行是进位，左边是高位，右边是低位）
